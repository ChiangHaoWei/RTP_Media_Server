...
class ClientWindow(QMainWindow):
    ...
    def __init__(
        ...
        self.positionSlider.sliderPressed.connect(self.sliderpress)
        self.positionSlider.sliderReleased.connect(self.sliderrelease)
    ...
    def sliderrelease(self):
        if abs(self._media_client.time_stamp_v - self.positionSlider.value()) > 20:
            self._media_client.stream_player.stop_stream()
            self._media_client.stream_player.close()
            self._media_client.stream_player = self._media_client.py_audio.open(
                format = self._media_client.py_audio.get_format_from_width(self._media_client.samplewidth_a),
            ...
            self._media_client.send_play_command(start=position)
            self._media_client.time_stamp_v = position
            self._media_client.time_stamp_a = int(position/self._media_client.fps_v * (self._media_client.fps_a/4410))
        else:
            self.handle_play()
        position = self.positionSlider.value()
        print(position)
